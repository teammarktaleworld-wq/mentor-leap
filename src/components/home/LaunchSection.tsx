...
//   useEffect(() => {
//     fetchEvents().then(data => {
//       const filtered = data.filter((ev: any) => ev.id === "interview-to-offer-letter");

//       // Fallback: if not in API, inject it manually
...
//                     : "none",
//                   opacity: visible ? 1 : 0,
//                   transition: `opacity 0.6s ease 0.2s, transform 0.35s ease, border-color 0.35s ease, box-shadow 0.35s ease`,
//                 }}
//               >
//                 <div className="ai-glow" />
...
//                   >
//                     By {event.speaker || "Mridu Bhandari"}
//                   </p>
...
const FALLBACK_EVENT = {
  ...
  title: "Interview to Offer Letter: The Ultimate Communication Masterclass",
  description:
    "Learn how to answer the most commonly asked interview questions with clarity, structure, and confidence. Turn interviews into offer letters.",
  price: 499,
  speaker: "Mridu Bhandari",
  date: "2026-04-30",
};
    ...
    fetchEvents()
      .then((data) => {
        const filtered: any[] = data.filter(
          (ev: any) => ev.id === "interview-to-offer-letter"
        );

        // Fallback: if not in API, inject it manually
        if (filtered.length === 0) {
          filtered.push(FALLBACK_EVENT);
        }
  ...
  return (
    <>
      ...
      >
        ...
        {/* COUNTDOWN — uncomment to re-enable */}
        {/* {visible && (
          <div className="countdown-animate mt-20">
            <h4
              className="text-white font-semibold mb-6"
