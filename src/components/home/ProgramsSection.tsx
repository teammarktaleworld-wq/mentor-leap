...
export default function ProgramsSection() {
  ...
  useEffect(() => {
    fetchEvents().then(data => {
      ...
      // Only show the Interview to Offer Letter event
      setEvents(data.filter((ev: any) => ev.id === "interview-to-offer-letter"));
      setLoading(false);
    });
  }, []);
        ...
        {!loading && events.length > 0 && (
          ...
          >
            {events.map((event, i) => (
              <div
                key={event.id}
                className="relative rounded-2xl text-left premium-card-glow"
                style={{
                  background: "#020617",
                  padding: "40px",
              ...
              >
                ...
                <p className="mb-4" style={{ color: "#6366f1", fontSize: "14px" }}>By {event.speaker || 'Mridu Bhandari'}</p>
                <p className="mb-6" style={{ color: "#cbd5f5", fontSize: "14px", lineHeight: "1.6" }}>
                  {event.description}
                </p>
                <div className="mb-4" style={{ height: "1px", background: "rgba(255,255,255,0.06)" }} />
